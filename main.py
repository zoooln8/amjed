...
# --- نظام الطلب مع اختيار طريقة الدفع ---
...
async def get_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إضافة خطوة اختيار وسيلة الدفع
    pay_keyboard = [['BOK', 'My Cashy']]
    pay_markup = ReplyKeyboardMarkup(pay_keyboard, resize_keyboard=True, one_time_keyboard=True)
    await update.message.reply_text("💳 اختر طريقة الدفع التي استخدمتها:", reply_markup=pay_markup)
...
async def get_pay_method(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📸 ممتاز، أرسل الآن لقطة شاشة للتحويل عبر ({update.message.text}):")
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "accept":
        ...
        await context.bot.send_message(chat_id=client_id, text=f"🎉 تم قبول طلبك رقم (#{order_num}).\nسيصلك الكود الآن ❤️")
    elif action == "reject":
        await query.edit_message_caption(caption=query.message.caption + "\n\n❌ **الحالة: مرفوض.**")
...
if __name__ == '__main__':
    ...
    app.add_handler(MessageHandler(filters.Text('💳 طرق الدفع'), lambda u, c: u.message.reply_text("💳 طرق الدفع المتوفرة:\n✅ بنكك BOK\n✅ My Cashy", parse_mode='Markdown')))
